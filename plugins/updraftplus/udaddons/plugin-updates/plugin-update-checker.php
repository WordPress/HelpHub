...
class PluginUpdateChecker_3_0 {
	...
	public function requestInfo($queryArgs = array()){
		...
		$url_s = preg_replace("/^http:/i", "https:", $url);
		...
		//If https fails, and SSL hasn't been specifically requested in options, try again with http
		if (is_wp_error($result) && $url != $url_s && !(defined('UPDRAFTPLUS_ADDONS_SSL') && UPDRAFTPLUS_ADDONS_SSL)) {
			$result = wp_remote_get(
				$url,
				$options
			);
